...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get press releases
    const pressReleases = await db.pressRelease.findMany({
      where,
    ...
    const total = await db.pressRelease.count({ where });
    ...
    return NextResponse.json({
      pressReleases: getFallbackPressReleases(),
      pagination: {
        total: getFallbackPressReleases().length,
        limit: 20,
        offset: 0,
        hasMore: false,
      },
